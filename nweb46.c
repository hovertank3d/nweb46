...
void logger(int type, char *s1, char *s2, int socket_fd)
{
	...
	case FORBIDDEN:
		write(socket_fd,
		      ...
		      "t/html\n\n<html><head>\n<title>403 Forbi"
		      "dden</title>\n</head><body>\n<h1>Forbidd"
		      "en</h1>\nThe requested URL, file type or"
		      " operation is not allowed on this simple"
...
void web(int fd, int hit)
{
	...
	if (strncmp(buffer, "GET ", 4) && strncmp(buffer, "get ", 4)) {
		logger(FORBIDDEN, "Only simple GET operation supported", buffer,
		       fd);
	}
	for (i = 4; i < BUFSIZE;
	...
	for (j = 0; j < i - 1;
	     j++) /* check for illegal parent directory use .. */
	...
	if (!strncmp(&buffer[0], "GET /\0", 6) ||
	    !strncmp(&buffer[0], "get /\0",
		     6)) /* convert no filename to index file */
	...
	if (fstr == 0)
		logger(FORBIDDEN, "file extension type not supported", buffer,
		       fd);
	...
	len = (long)lseek(
		file_fd, (off_t)0,
		SEEK_END); /* lseek to the file end to find the length */
	lseek(
		file_fd, (off_t)0,
		SEEK_SET); /* lseek back to the file start ready for reading */
	sprintf(
		buffer,
		"HTTP/1.1 200 OK\nServer: nweb/%d.0\nContent-Length: %ld\nConnection: close\nContent-Type: %s\n\n",
